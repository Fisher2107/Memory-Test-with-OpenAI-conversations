...
def get_message_time(message):
    """
    Extract the time from a message content.
...
def get_conversation_messages(conversation):
    ...
    while current_node:
        ...
        if message:
            ...
            time = get_message_time(message)
            if parts and len(parts) > 0 and len(parts[0]) > 0:
                if author != "system" or message.get("metadata", {}).get(
                    "is_user_system_message"
                ):
                    messages.append({"author": author, "text": parts[0], "time": time})
...
def update_conversation_summary(summary, directory_name, conversation, date, messages):
    ...
    summary[directory_name].append(
        {
            "title": conversation.get("title", "Untitled") or "Conversation between user and agent",
...
def parse(input_file: str, output_folder: str):
    ...
    created_directories_info = write_conversations_and_summary(
        conversations_data, output_folder
    )
